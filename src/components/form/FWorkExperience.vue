...
<template>
  <div>
    <div class="split-row-module" v-for="(data, index) in datas">
      <el-row class="split-row-1">
        ...
        <el-col :span="8">
          <div class="split-1">
            <label>在职时间</label>
            <el-date-picker :clearable="false" v-model="data.startAt" style="width: 130px" type="month" placeholder="开始时间" value-format="YYYY-MM" />
            -
            <el-date-picker :clearable="false" v-model="data.endAt" style="width: 130px" type="month" placeholder="结束时间" value-format="YYYY-MM" v-if="!data.toNow" />
      ...
      <el-row class="split-row-1">
        <el-col>
          <div class="split-1">
            <RichText v-model="data.content" placeholder="详细描述你的职责范围、工作任务及取得的成绩，描述尽量具体简洁，工作经验的描述尽量与目标岗位的招聘要求匹配。" />
          </div>
        </el-col>
        <el-col>
    ...
    <el-row>
      <el-col>
        <el-button @click="onPlusWork" type="success" size="large" :icon="plus" round>新增一条工作经历</el-button>
      </el-col>
    </el-row>
  </div>
</template>

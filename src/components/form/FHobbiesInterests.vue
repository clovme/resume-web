...
function addCustomTag() {
  if (!customTag.value || customTag.value.trim() === '') {
    ElNotification({title: '提示信息', message: '自定义名称不能为空。', type: 'warning'})
    return
  }
...
<template>
  <div>
    <el-row class="split-row-2">
      <el-col :span="24">
        <div class="split-2">
          <RichText placeholder="兴趣爱好内容描述，非必填。" v-model="data.content"/>
        </div>
      </el-col>
      <el-col :span="24">
        <div class="split-2">
      ...
      <el-col :span="8">
        <div class="split-2">
          <el-input v-model="customTag" @keyup.enter="addCustomTag" placeholder="请输入自定义名称" clearable/>
          <el-button class="el-c-button" :icon="plus" plain @click="addCustomTag">添加自定义兴趣爱好</el-button>
        </div>
      </el-col>
    </el-row>
